...
create table flashcard(
    ...
    primary key (codigo_flashcard, codigo_usuario, e_mail),
...
create table edita(
    ...
    e_mail varchar(50) not null,
	primary key (codigo_flashcard, codigo_usuario, e_mail),
    constraint foreign key(codigo_flashcard)
    references flashcard(codigo_flashcard),
    constraint foreign key(codigo_usuario)
    references flashcard(codigo_usuario),
    constraint foreign key(e_mail)
    references usuario(e_mail)) engine=innodb;
